...
//recording and logging data -------------------------------------------------------------------------------------------------------------------------------------------------- NEEDS TESTING
  ...
  if (millis() - lastTime > 500)                               //GPS Data (Lat, Long, SIV)
  {
    lastTime = millis();
    ...
    long latitude = myGNSS.getLatitude();       //Has been tested and works, leaving call but removing print
    long longitude = myGNSS.getLongitude();       //Has been tested and works, leaving call but removing print
    byte SIV = myGNSS.getSIV();         //Has been tested and works, leaving call but removing print
